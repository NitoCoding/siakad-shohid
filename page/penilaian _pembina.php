...
<tr>
  <td>&nbsp;</td>
  <td><strong>PENILAIAN PEMBINA ASRAMA PPMI SHOHWATUL IS'AD </strong></td>
  <td><div align="right">
    <? if($idkusr=="d"){ ?>
    ( <a href="leger_penilaian_pegawai.php?qbln=<? echo $qbln; ?>&tkt=<? echo $tkt; ?>" target="_blank">Rekap Penilaian</a> )
    <? } ?>
  </div></td>
</tr>
<tr>
...
<tr>
  <td height="31">&nbsp;</td>
  <td colspan="2" valign="middle" bgcolor="#663300"><span class="style1">&nbsp;&nbsp;Kesantrian </span>
    
      <select name="menu2" onchange="MM_jumpMenu('parent',this,0)">
	   <option value="penilaian _pembina.php">-</option>
        <option value="penilaian _pembina.php?tkt=KSP&idgp=<? echo $tp['id_gp']; ?>" <? if($tkt=="KSP"){ echo"selected='selected'"; } ?>>Putra</option>
		<option value="penilaian _pembina.php?tkt=KSI&idgp=<? echo $tp['id_gp']; ?>" <? if($tkt=="KSI"){ echo"selected='selected'"; } ?>>Putri</option>
      </select>
    
    <span class="style1">Pembina </span>   &nbsp; <select name="menu1" onchange="MM_jumpMenu('parent',this,0)">
      <option value="penilaian _pembina.php?tkt=<? echo $tkt; ?>">-</option>
      <? 
	      
		  $tampil2=mysqli_query($koneksi, "SELECT * FROM guru_pegawai WHERE unit='$tkt'  ");
          while($tp=mysqli_fetch_assoc($tampil2)){
	    ?>
      <option value="penilaian _pembina.php?tkt=<? echo $tkt; ?>&idgp=<? echo $tp['id_gp']; ?>" <? if($idgp=="$tp[id_gp]"){ echo"selected='selected'"; } ?>><? echo $tp['nama_gp']; ?></option>
      <? } ?>
    </select> 
    &nbsp;<span class="style2">Bulan </span>
    <select name="select" onchange="MM_jumpMenu('parent',this,0)">
      <option value="penilaian _pembina.php?idgp=<? echo $idgp; ?>&tkt=<? echo $tkt; ?>">-</option>
      <?  
	      
		 for($i=1;$i<=12;$i++){
	    ?>
      <option value="penilaian _pembina.php?tkt=<? echo $tkt; ?>&idgp=<? echo $idgp; ?>&qbln=<? echo $i; ?>" <? if($qbln=="$i"){ echo"selected='selected'"; } ?>><? echo $i; ?></option>
      <? } ?>
    </select>     &nbsp;&nbsp;<a href="raport_bulanan_pembina.php?idgp=<? echo $idgp; ?>&qbln=<? echo $qbln; ?>&tkt=<? echo $tkt; ?>" class="lk4" target="_blank">Cetak Raport</a></td>
  </tr>
<tr>
  <td width="21">&nbsp;</td>
  <td width="591">&nbsp;</td>
  ...
  <td colspan="2"><form id="form1" name="form1" method="post" action="act_penilaian_guru.php">
    <table width="100%" border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
		...
		<?
		  $tampi4=mysqli_query($koneksi, "SELECT * FROM tb_penilaian_bulanan_pegawai WHERE id_guru='$idgp' and thn_ajaran='$thaj' and bulan='$qbln'");
      ...
      <tr>
        ...
        <td bgcolor="#EEEEEE">&nbsp;</td>
        <td bgcolor="#EEEEEE">&nbsp;</td>
        <td rowspan="11" align="center" bgcolor="#EEEEEE"><input type="submit" name="spp" value="Simpan" <? if($qbln==NULL){ echo "disabled='disabled'"; }  ?>/></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td bgcolor="#EEEEEE">Komunikasi yang baik ke santri, orang tua wali, sesama pembina dan Pimpinan</td>
      ...
      <tr>
        ...
        <td bgcolor="#999999"><span class="style10">Tertib administrasi pembina (Laporan perkembangan santri) </span></td>
